...
def run(fold, args):
    ...
    train_images = np.load(args.train_images, allow_pickle=True) 
    train_image_paths = [os.path.join(args.train_data_dir, image_name+'.jpg') for image_name in train_images]
    train_targets = np.load(args.train_targets,  allow_pickle=True)
    ...
    valid_images = np.load(args.valid_images,  allow_pickle=True) 
    valid_image_paths = [os.path.join(args.valid_data_dir, image_name+'.jpg') for image_name in valid_images]
    ...
    for epoch in range(args.epochs):
        train_loss = train_one_epoch(args, train_loader, model, optimizer, weights=weights)
...
def main():
    ...
    parser.add_argument('--train_images', default=None)
    parser.add_argument('--train_targets', default=None)
    parser.add_argument('--valid_images', default=None)
    parser.add_argument('--valid_targets', default=None)
    parser.add_argument('--training_folds_csv', default=None)
